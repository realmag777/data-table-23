...
class DataTable23 {
    constructor(table_data, table_html_id, additional = {}) {
        ...
        if (!this.request_data.orderby) {
            this.request_data.orderby = this.settings.orderby;
            this.request_data.order = this.settings.order;
        }

        if (typeof this.request_data.per_page === 'undefined') {
            this.request_data.per_page = this.settings.per_page;
            this.request_data.current_page = this.settings.current_page;
        }
    ...
    build_rows(data) {
        ...
        if (!max_iterations) {
            max_iterations = parseInt(this.settings.per_page);
        }
        ...
        for (let x = 0; x < data.length && x < max_iterations; x++) {
            ...
            Object.keys(data[x]).forEach(function (current_field) {
                ...
                if (typeof _this.settings.draw_cells !== 'undefined') {
                    if (_this.settings.draw_cells[current_field]) {
                        td.innerHTML='';
                        td.appendChild(_this.settings.draw_cells[current_field](data[x][current_field], td, _this));
                    }
                }
